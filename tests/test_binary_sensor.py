...
class TestBinarySensorMissingPaths:
    ...
    def test_is_on_appliance_state_running(self, mock_coordinator, mock_capability):
        """applianceState RUNNING → is_on True."""
        entity = self._make_entity(
            mock_coordinator, mock_capability, "applianceState", "appliancestate"
        )
        entity.is_connected = MagicMock(return_value=True)
        entity.extract_value = MagicMock(return_value="RUNNING")
        assert entity.is_on is True

    def test_is_on_appliance_state_off(self, mock_coordinator, mock_capability):
        """applianceState OFF → is_on False."""
        entity = self._make_entity(
            mock_coordinator, mock_capability, "applianceState", "appliancestate"
        )
        entity.is_connected = MagicMock(return_value=True)
        entity.extract_value = MagicMock(return_value="OFF")
        assert entity.is_on is False

    def test_is_on_appliance_state_idle(self, mock_coordinator, mock_capability):
        """applianceState IDLE → is_on True (appliance on but idle)."""
        entity = self._make_entity(
            mock_coordinator, mock_capability, "applianceState", "appliancestate"
        )
        entity.is_connected = MagicMock(return_value=True)
        entity.extract_value = MagicMock(return_value="IDLE")
        assert entity.is_on is True

    def test_is_on_appliance_state_paused(self, mock_coordinator, mock_capability):
        """applianceState PAUSED → is_on True."""
        entity = self._make_entity(
            mock_coordinator, mock_capability, "applianceState", "appliancestate"
        )
        entity.is_connected = MagicMock(return_value=True)
        entity.extract_value = MagicMock(return_value="PAUSED")
        assert entity.is_on is True

    def test_is_on_appliance_state_end_of_cycle(self, mock_coordinator, mock_capability):
        """applianceState END_OF_CYCLE → is_on True."""
        entity = self._make_entity(
            mock_coordinator, mock_capability, "applianceState", "appliancestate"
        )
        entity.is_connected = MagicMock(return_value=True)
        entity.extract_value = MagicMock(return_value="END_OF_CYCLE")
        assert entity.is_on is True
